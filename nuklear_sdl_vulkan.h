...
nk_sdl_vk_init(SDL_Window *win, VkDevice logical_device,
               ...
               VkDeviceSize max_element_buffer, VkQueue graphics_queue);
NK_API void nk_sdl_vk_shutdown(void);
NK_API void nk_sdl_vk_font_stash_begin(struct nk_font_atlas **atlas);
NK_API void nk_sdl_vk_font_stash_end(void);
...
NK_API void
nk_sdl_vk_device_create(VkDevice logical_device, VkPhysicalDevice physical_device,
...
NK_INTERN unsigned int nuklearshaders_nuklear_vert_spv_len = 1856;
NK_INTERN unsigned char nuklearshaders_nuklear_frag_spv[] = {
    0x03, 0x02, 0x23, 0x07, 0x00, 0x00, 0x01, 0x00, 0x0b, 0x00, 0x0d, 0x00,
...
NK_INTERN unsigned int nuklearshaders_nuklear_frag_spv_len = 860;
...
NK_API
void nk_sdl_vk_shutdown(void) {
    nk_font_atlas_clear(&sdl.atlas);
    nk_free(&sdl.ctx);
    nk_sdl_vk_device_destroy();
...
NK_API void nk_sdl_vk_font_stash_end(void) {
    ...
    nk_sdl_device_upload_atlas(sdl.graphics_queue, image, w, h);
...
NK_API
VkSemaphore nk_sdl_vk_render(uint32_t buffer_index,
                             VkSemaphore wait_semaphore,
                             enum nk_anti_aliasing AA) {
    struct nk_sdl_device *dev = &sdl.vulkan;
    ...
    projection.m[0] /= sdl.width;
    projection.m[5] /= sdl.height;
    ...
    viewport.width = (float)sdl.display_width;
    viewport.height = (float)sdl.display_height;
                            ...
                            &dev->uniform_descriptor_set, 0, NULL);
    {
        ...
        nk_draw_foreach(cmd, &sdl.ctx, &dev->cmds) {
            ...
            scissor.offset.x = (int32_t)(NK_MAX(cmd->clip_rect.x * sdl.scale, 0.f));
            scissor.offset.y = (int32_t)(NK_MAX(cmd->clip_rect.y * sdl.scale, 0.f));
            scissor.extent.width = (uint32_t)(cmd->clip_rect.w * sdl.scale);
            scissor.extent.height = (uint32_t)(cmd->clip_rect.h * sdl.scale);
    ...
    result = vkQueueSubmit(sdl.graphics_queue, 1, &submit_info, NULL);
    NK_ASSERT(result == VK_SUCCESS);
...
nk_sdl_vk_init(SDL_Window *win, VkDevice logical_device,
    ...
    sdl.win = win;
    sdl.graphics_queue = graphics_queue;
    ...
    sdl.scale = (scale_x + scale_y) * 0.5;

    nk_sdl_vk_device_create(logical_device, physical_device,
